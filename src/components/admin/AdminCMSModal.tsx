...
export const AdminCMSModal: React.FC<AdminCMSModalProps> = ({ onLogout }) => {
  ...
  const [registrationsPerPage, setRegistrationsPerPage] = useState<number>(10);

  // Computed Filtered & Paginated Registrations
  const filteredRegistrations = useMemo(() => {
    return cmsData.registrations.filter((r) => {
  ...
  }, [cmsData.registrations, registrationFilter, eventFilter, registrationSearch]);

  const totalRegistrationPages = Math.max(1, Math.ceil(filteredRegistrations.length / registrationsPerPage));
  const safeRegistrationPage = Math.min(Math.max(1, registrationPage), totalRegistrationPages);
  const startRegIndex = (safeRegistrationPage - 1) * registrationsPerPage;
  const endRegIndex = Math.min(startRegIndex + registrationsPerPage, filteredRegistrations.length);
  const paginatedRegistrations = useMemo(() => {
    return filteredRegistrations.slice(startRegIndex, endRegIndex);
  }, [filteredRegistrations, startRegIndex, endRegIndex]);
            ...
            {activeAdminTab === 'registrations' && (
              <div className="space-y-6">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div className="flex items-center gap-1 p-1 rounded-xl bg-slate-100 border border-slate-200/80 overflow-x-auto no-scrollbar">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          setRegistrationFilter('business');
                          setRegistrationPage(1);
                        }}
                        className={`px-3 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                ...
                <div className="bg-white rounded-2xl border border-slate-200/90 shadow-xs overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-xs">
                      ...
                      <tbody className="divide-y divide-slate-100 text-slate-700">
                        {paginatedRegistrations.map((attendee) => {
                          ...
                          <tr>
                            <td colSpan={8} className="px-4 py-10 text-center text-slate-400 text-xs">
                              {cmsData.registrations.length === 0
                                ...
                                : 'Không tìm thấy đại biểu nào phù hợp với bộ lọc tìm kiếm.'}
                            </td>
                          </tr>
                        )}
                      </tbody>
                  ...
                  {/* Pagination Controls */}
                  <div className="p-3.5 sm:p-4 bg-slate-50/80 border-t border-slate-200/80 flex flex-col md:flex-row items-center justify-between gap-3 text-xs">
                    {/* Left: Records Info & Per Page selector */}
                    <div className="flex flex-wrap items-center gap-3 text-slate-600 w-full md:w-auto justify-between md:justify-start">
                      <div>
                        {filteredRegistrations.length > 0 ? (
                          <span>
                            Hiển thị <strong className="text-slate-900">{startRegIndex + 1}</strong> – <strong className="text-slate-900">{endRegIndex}</strong> trong tổng số <strong className="text-slate-900">{filteredRegistrations.length}</strong> đại biểu
                            {filteredRegistrations.length !== cmsData.registrations.length && (
                              <span className="text-slate-400 ml-1">(lọc từ {cmsData.registrations.length})</span>
                            )}
                          </span>
                        ) : (
                          <span>Không có đại biểu phù hợp</span>
                        )}
                      </div>

                      <div className="flex items-center gap-1.5 pl-2 md:border-l md:border-slate-200">
                        <span className="text-[11px] text-slate-500 font-medium">Mỗi trang:</span>
                        <select
                          ...
                          className="px-2 py-1 rounded-lg border border-slate-200 bg-white text-xs font-bold text-slate-700 outline-none focus:border-[#174ea6] cursor-pointer"
                        >
                    ...
                    {totalRegistrationPages > 1 && (
                      <div className="flex items-center gap-1 w-full md:w-auto justify-center md:justify-end">
                        ...
                        <div className="flex items-center gap-1 px-1">
                          {Array.from({ length: totalRegistrationPages }, (_, i) => i + 1)
                            .filter((pageNum) => {
                              if (totalRegistrationPages <= 7) return true;
                              if (pageNum === 1 || pageNum === totalRegistrationPages) return true;
                              if (Math.abs(pageNum - safeRegistrationPage) <= 1) return true;
                              return false;
                            })
                            .map((pageNum, idx, arr) => {
                        ...
                        <button
                          type="button"
                          disabled={safeRegistrationPage >= totalRegistrationPages}
                          onClick={() => setRegistrationPage((p) => Math.min(totalRegistrationPages, p + 1))}
                          className="p-1.5 rounded-lg border border-slate-200 bg-white hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed text-slate-600 transition-colors cursor-pointer"
                          title="Trang sau"
                        >
                        ...
                        <button
                          type="button"
                          disabled={safeRegistrationPage >= totalRegistrationPages}
                          onClick={() => setRegistrationPage(totalRegistrationPages)}
                          className="p-1.5 rounded-lg border border-slate-200 bg-white hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed text-slate-600 transition-colors cursor-pointer"
                          title={`Đến trang cuối (Trang ${totalRegistrationPages})`}
                        >
