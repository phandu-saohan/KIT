          ...
          {expertsList.map((expert) => {
            ...
            return (
              <div
                ...
                className="group relative bg-white rounded-3xl border border-slate-200/90 hover:border-[#c83271]/50 shadow-xs hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex flex-row overflow-hidden cursor-pointer"
              >
                ...
                <div className="relative w-32 xs:w-36 sm:w-48 md:w-52 shrink-0 bg-gradient-to-b from-slate-100 to-slate-200 overflow-hidden">
                  <img
                  ...
                  <div className="absolute top-2.5 left-2.5 z-10">
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-white/95 backdrop-blur-md text-slate-800 shadow-xs border border-white/80">
                      <span>🇰🇷</span>
                      <span className="text-[9.5px] text-slate-500 font-semibold">HQ</span>
                    </span>
                  </div>
                  ...
                  <div className="absolute bottom-2 left-2 right-2 z-10">
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] font-bold shadow-xs truncate max-w-full ${
                        isKeynote
                ...
                <div className="p-3.5 sm:p-5 flex flex-col justify-between flex-1 min-w-0 gap-2.5 sm:gap-3">
                  <div>
                    {/* Organization Pill */}
                    <div className="flex items-center gap-1.5 mb-1.5">
                      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10.5px] font-bold uppercase tracking-wide bg-[#eff4ff] text-[#174ea6] border border-[#d0e1fd] truncate max-w-full">
                        <Building2 className="w-3 h-3 text-[#174ea6] shrink-0" />
                        <span className="truncate">{expert.badgeOrg}</span>
                      </span>
                    </div>

                    {/* Doctor Name */}
                    <h3 className="text-[16px] sm:text-[19px] font-black text-slate-900 font-display group-hover:text-[#c83271] transition-colors leading-tight">
                      {expert.name}
                    </h3>
                  </div>

                  {/* Key Credentials Preview (2 points) */}
                  <div className="space-y-1.5 bg-[#f8faff] rounded-xl p-2.5 border border-slate-100 flex-1 flex flex-col justify-center">
                    {expert.bioPoints.slice(0, 2).map((point, i) => (
                      <div key={i} className="flex items-start gap-1.5 text-[11px] sm:text-[12px] text-slate-600 leading-snug">
                        <CheckCircle2 className="w-3.5 h-3.5 text-[#174ea6] shrink-0 mt-0.5" />
                  ...
                  {/* Action Footer */}
                  <div className="border-t border-slate-100 pt-2 flex items-center justify-between text-[11.5px] font-bold">
                    <span className="text-[10.5px] sm:text-[11px] text-slate-400 font-normal truncate">
                      {expert.bioPoints.length} công trình khoa học
                    </span>

                    <span className="inline-flex items-center gap-1 text-[#174ea6] group-hover:text-[#c83271] transition-colors font-bold text-[11px] sm:text-[12px] shrink-0">
